...
#         # Convert AMOUNT_LC to numeric
#         time_series = pd.to_numeric(data['AMOUNT_LC'], errors='coerce').dropna()
...
def run_sarima_model(gl_account, description, profit_ctr, company_code):
    try:
        ...
        data = hana_df[
            (hana_df['PROFIT_CTR'] == 'PC1') &
            (hana_df['DESCRIPTION'] == 'Income 1') &
            (hana_df['COMPANY_CODE'] == 1000) &
            (hana_df['GL_ACCOUNT'] == 600001)
        ]
        ...
        data['AMOUNT_LC'] = pd.to_numeric(data['AMOUNT_LC'], errors='coerce')
        data = data.dropna(subset=['AMOUNT_LC'])
        for index, row in data.iterrows():
            forecast_summary_1.append({
                "Date": index.strftime('%Y-%m-%d'),
                "Forecast": round(row['AMOUNT_LC'], 2),
                "Lower_CI": 0.0,
                "Upper_CI": 0.0
            })

        print(forecast_summary_1)
